...
func Stream2Handlers(app core.App, router pyrin.Router) {
	g := router.Group("/api/stream2")
	g.Register(
		pyrin.NormalHandler{
			Name:   "GetPlaylist",
			Method: http.MethodGet,
			Path:   "/:mediaId/index.m3u8",
			HandlerFunc: func(c pyrin.Context) error {
				mediaId := c.Param("mediaId")

				query := c.Request().URL.Query()
				audio, _ := strconv.Atoi(query.Get("audio"))
				subtitle, _ := strconv.Atoi(query.Get("subtitle"))

				ctx := context.TODO()

				media, err := app.DB().GetMediaById(ctx, mediaId)
				if err != nil {
				...
				for leftover > 0 {
					...
					u := ConvertURL(c, fmt.Sprintf("/api/stream2/%v/segment%d.ts?audio=%d&subtitle=%d", mediaId, segmentIndex, audio, subtitle))
					fmt.Fprint(w, u+"\n")
		...
		pyrin.NormalHandler{
			...
			HandlerFunc: func(c pyrin.Context) error {
				...
				args := []string{
					...
					"-map", fmt.Sprintf("0:V:%d", videoTrack),
