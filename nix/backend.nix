...
{ config, lib, pkgs, ... }:
with lib; let
  cfg = config.services.nosepass;

  nosepassConfig = pkgs.writeText "config.toml" ''
    listen_addr = "${cfg.host}:${toString cfg.port}"
    data_dir = "/var/lib/nosepass"
...
in
{
  options.services.nosepass = {
    enable = mkEnableOption "Enable the nosepass service";
  ...
  config = mkIf cfg.enable {
    systemd.services.nosepass = {
      ...
      serviceConfig = {
        ...
        StateDirectory = "nosepass";

        ExecStart = "${cfg.package}/bin/nosepass serve -c '${nosepassConfig}'";
    ...
    users.users = mkIf (cfg.user == "nosepass") {
      nosepass = {
        group = cfg.group;
        isSystemUser = true;
      };
    ...
    users.groups = mkIf (cfg.group == "nosepass") {
      nosepass = {};
    };
  };
}
